...
--create database SmartAccountSaleVendor;
go
 --use SmartAccountSaleVendor;
...
CREATE TABLE Accounts (
 ...
 FOREIGN KEY (AccountParentID) REFERENCES Accounts(AccountID),
  
    FOREIGN KEY (NormalBalanceID) REFERENCES NormalBalances(BalanceID)

);

-- Create the JournalType table
CREATE TABLE JournalType (
    JournalTypeID INT PRIMARY KEY,
...
CREATE TABLE Journal (
    ...
    JournalDate DATE,
    Description VARCHAR(255),
    FOREIGN KEY (JournalTypeID) REFERENCES JournalType(JournalTypeID)
);
...
CREATE TABLE TransactionEntries (
    TransactionEntryID INT PRIMARY KEY,
...
-- Create the Purchase-related tables
...
CREATE TABLE PurchaseOrders (
    ...
    -- Add other purchase-related attributes
...
CREATE TABLE PurchaseOrderItems (
    ...
    -- Add other purchase order item attributes
    FOREIGN KEY (PurchaseOrderID) REFERENCES PurchaseOrders(PurchaseOrderID),
    FOREIGN KEY (ProductID) REFERENCES Product(ProductID)
);

-- Create the Sales-related tables
CREATE TABLE Customers (
    CustomerID INT PRIMARY KEY,
    CustomerName VARCHAR(100) NOT NULL,
    -- Add other customer-related attributes
    AccountID INT,
    FOREIGN KEY (AccountID) REFERENCES Accounts(AccountID)
);
...
CREATE TABLE SalesOrders (
    ...
    OrderDate DATE NOT NULL,
    TotalAmount DECIMAL(18, 2),
    -- Add other sales-related attributes
...
CREATE TABLE SalesOrderItems (
    ...
    Quantity INT,
    UnitPrice DECIMAL(18, 2),
    -- Add other sales order item attributes
    FOREIGN KEY (SalesOrderID) REFERENCES SalesOrders(SalesOrderID),
    FOREIGN KEY (ProductID) REFERENCES Product(ProductID)
);
-- Create the FinancialStatements table
CREATE TABLE FinancialStatements (
    StatementID INT PRIMARY KEY,
    StatementName VARCHAR(100),
    StartDate DATE,
    EndDate DATE
);

-- Create the StatementLines table
CREATE TABLE StatementLines (
    StatementLineID INT PRIMARY KEY,
    StatementID INT,
    AccountID INT,
    Balance DECIMAL(18, 2),
    FOREIGN KEY (StatementID) REFERENCES FinancialStatements(StatementID),
    FOREIGN KEY (AccountID) REFERENCES Accounts(AccountID)
);
...
CREATE TABLE PurchasePayments (
    ...
    InvoiceID INT FOREIGN KEY REFERENCES PurchaseInvoices (InvoiceID),
    PaymentDate DATE,
    Amount DECIMAL(18, 2)
);
...
CREATE TABLE StockAdjustmentItems (
    AdjustmentItemID INT PRIMARY KEY,
...
CREATE TABLE StoreInventory (
    StoreInventoryID INT  identity,
    StoreID INT,
    ItemID INT,
    QuantityOnHand INT,
    PRIMARY KEY (StoreInventoryID ,StoreID, ItemID),
    FOREIGN KEY (StoreID) REFERENCES Store(StoreID),
    FOREIGN KEY (ItemID) REFERENCES Item(ItemID)
);

-- Create the WarehouseInventory table
...
CREATE TABLE InventoryMetrics (
    ...
    CarryingCost DECIMAL(10, 2),
    StockoutRate DECIMAL(5, 2),
    EOQ INT,
    OrderCycleTime INT,
    ReplenishmentLeadTime INT,
    FOREIGN KEY (ItemID) REFERENCES Item(ItemID)
);


CREATE TABLE ReplenishmentInfo (
...
CREATE TABLE InventorySnapshot (
    SnapshotID INT PRIMARY KEY IDENTITY,
   ItemID INT,
    QuantityOnHand INT,
...
-- StockReceiptItems table
CREATE TABLE StockReceiptItems (
    ReceiptItemID INT PRIMARY KEY,
    ReceiptID INT FOREIGN KEY REFERENCES StockReceipts (ReceiptID),
    ProductID INT FOREIGN KEY REFERENCES Product (ProductID),
    Quantity INT
);
...
-- StockIssueItems table
CREATE TABLE StockIssueItems (
    IssueItemID INT PRIMARY KEY,
    IssueID INT FOREIGN KEY REFERENCES StockIssues (IssueID),
    ProductID INT FOREIGN KEY REFERENCES Product (ProductID),
    Quantity INT
);
